...
class StationDetailsContent extends StatelessWidget {
  ...
  bool get _hasThaiName => stop.thaiName != null && stop.thaiName!.trim().isNotEmpty;
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      padding: EdgeInsets.fromLTRB(16, isBottomSheet ? 8 : 16, 16, MediaQuery.of(context).padding.bottom + 24),
      child: Column(
        ...
        children: [
          ...
          if (stop.desc != null && stop.desc!.isNotEmpty) ...[
            ...
            Text(stop.desc!, style: theme.textTheme.bodyMedium?.copyWith(color: scheme.onSurfaceVariant)),
            const SizedBox(height: 24),
          ],
          if (transferStops.isNotEmpty) ...[
            _SectionHeader('Transfers'),
            const SizedBox(height: 12),
            _buildTransfersList(context, theme, scheme),
            const SizedBox(height: 24),
          ],
  ...
  Widget _buildHeroCard(BuildContext context, ThemeData theme, ColorScheme scheme) {
    return Container(
                  ...
                  Padding(
                    ...
                    child: Wrap(
                      ...
                      children: lineName!.split(', ').map((singleLine) {
                        final specificColor = lineColorByName?.call(singleLine) ?? lineColor;
                        return Container(
  ...
  Widget _buildTransfersList(BuildContext context, ThemeData theme, ColorScheme scheme) {
            ...
            child: Row(
              children: [
                Container(
                  width: 12,
                  height: 12,
                  decoration: BoxDecoration(color: tLineColor, shape: BoxShape.circle),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                      ...
                      Wrap(
                        ...
                        children: tLineName.split(', ').map((sl) {
                          ...
                          return Container(
                            ...
                            child: Text(
                              sl,
                              style: theme.textTheme.bodySmall?.copyWith(color: slColor, fontWeight: FontWeight.w600),
                            ),
                          );
                        }).toList(),
                      ),
  ...
  Widget _buildInfoChips(BuildContext context, ThemeData theme, ColorScheme scheme) {
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: [
        ...
        _InfoChip(
          label: 'Coordinates',
          value: '${stop.lat.toStringAsFixed(4)}, ${stop.lon.toStringAsFixed(4)}',
        ),
      ],
    );
  }
