...
def match_shapes_to_ferry_trips():
    print("Loading custom stops...")
    # 1. Load Custom Stops to get lat/lon for each stop_id
    stops = pd.read_csv('ferry_stop.txt', dtype=str, on_bad_lines='skip')
    ...
    print("Loading ferry stop times...")
    # 3. Read ferry_stop_times.txt and find start/end stop for each trip
    stop_times = pd.read_csv('ferry_stop_times.txt', dtype=str)
    ...
    output_df.to_csv('ferry_trips.txt', index=False)

    print("Done! Check 'ferry_trips.txt'")

if __name__ == "__main__":
    match_shapes_to_ferry_trips()
